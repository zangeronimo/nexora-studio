...
import { useCategoryUpdateForm } from '../hooks/use-category-update.form';
...
export function CulinaryCategoryUpdatePage({ categoryService }: Props) {
  const { id } = useParams();
  const {
    errors,
    hasError,
  ...
  return (
    <Page title={t('culinary.category.update.title')}>
      <Card description={t('culinary.category.update.description')}>
